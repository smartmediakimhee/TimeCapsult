...
public class BoardMainGUI implements Runnable {
	...
	private synchronized void initialize() {
		...
		/*
		 * JLabel lbl_sort = new JLabel() { public void paintComponent(Graphics g) { //
		 * Approach 1: Dispaly image at at full size // g.drawImage(Mainicon.getImage(),
		 * 0, 0, null); // Approach 2: Scale image to size of component Dimension d =
		 * new Dimension(200, 200); g.drawImage(list.getImage(), 0, 0, d.width,
		 * d.height, null); // Approach 3: Fix the image position in the scroll pane //
		 * Point p = scrollPane.getViewport().getViewPosition(); //
		 * g.drawImage(icon.getImage(), p.x, p.y, null); setOpaque(false); // 그림을 표시하게
		 * 설정,투명하게 조절 super.paintComponent(g); } };
		 ...
		 * lbl_sort, 1600, SpringLayout.WEST, panel_big);
		 * 
		 * lbl_sort.addMouseListener(new MouseAdapter() {
		 * 
		 * @Override public void mouseClicked(MouseEvent e) { if (isSortboardArr) {
		 * JOptionPane.showMessageDialog(null, "게시글을 남은 열림시간순으로 정렬합니다");
		 * sortBoardArrtime(); isSortboardArr = false; } else { init_boardArr();
		 * isSortboardArr = true; } }
		 * 
		 * @Override public void mouseEntered(MouseEvent e) { lbl_sort.setCursor(new
		 * Cursor(12)); } });
		 */
		/*
		 * sl_panel_big.putConstraint(SpringLayout.NORTH, lbl_sort, 800,
		...
		// size.setSize(10, panelsHeightSize(4)); // panelHeightSize(int count) 에서 count
		// 는 페널들이 담긴 ArrayList의 size 값이다.
